...
/**
 * The {@code ClickHandler} class is used for the client to interact with the
 * Entity chat UI. This class helps
 * to receive messages from the server, cast rays to see what the user clicked
 * on, and communicate these events
 * back to the server.
 */
public class ClickHandler {
    ...
    public static boolean handleUseKeyClick(Minecraft client) {
        ...
        // Get the camera position for ray start to support both first-person and
        // third-person views
        ...
        for (Map.Entry<UUID, BubbleLocationManager.BubbleData> entry : BubbleLocationManager.getAllBubbleData()
                .entrySet()) {
            ...
            Vec3[] corners = getBillboardCorners(bubbleData.position, camera.getPosition(), bubbleData.height,
                    bubbleData.width, bubbleData.yaw, bubbleData.pitch);
        ...
        if (closestEntityUUID != null) {
            ...
            if (closestEntity != null) {
                ...
                EntityChatDataLight chatData = ClientChatDataManager
                        .getOrCreateChatData(closestEntityUUID.toString());

                // Determine area clicked inside chat bubble (top, left, right)
                String hitRegion = determineHitRegion(closestHitResult.get(), closestBubbleData.position, camera,
                        closestBubbleData.height);
                ...
                if (chatData.status == ChatDataManager.ChatStatus.NONE) {
                    ...
                    if (hitRegion.equals("RIGHT") && !chatData.isEndOfMessage()) {
                        ...
                        ClientPackets.sendUpdateLineNumber(closestEntity,
                                chatData.currentLineNumber + ChatDataManager.DISPLAY_NUM_LINES);
    ...
    public static Vec3[] getBillboardCorners(Vec3 center, Vec3 cameraPos, double height, double width, double yaw,
            double pitch) {
        ...
        // Calculate initial 'right' and 'up' vectors assuming 'up' is the global Y-axis
        // (0, 1, 0)
    ...
    public static Optional<Vec3> rayIntersectsTriangle(Vec3 rayOrigin, Vec3 rayDirection, Vec3 v0, Vec3 v1, Vec3 v2) {
        ...
        if (Math.abs(a) < 1e-6)
            return Optional.empty(); // Ray is parallel to the triangle
    ...
    public static String determineHitRegion(Vec3 hitPoint, Vec3 center, Camera camera, double height) {
        ...
        // Calculate the "RIGHT" vector as perpendicular to the 'toCamera' vector and
        // the global up vector
        Vec3 right = globalUp.cross(toCamera).normalize();

        // Handle the case where the camera is looking straight down or up, making the
        // cross product degenerate
        if (right.lengthSqr() == 0) {
            // If directly above or below, define an arbitrary right vector (assuming world
            // x-axis)
            right = new Vec3(1, 0, 0);
        }

        // Recalculate "UP" vector to ensure it's orthogonal to both "RIGHT" and
        // "toCamera"
        Vec3 up = toCamera.cross(right).normalize();

        // Calculate the relative position of the hit point to the center of the
        // billboard
        ...
        if (relY > 0.70 * height) {
            ...
            // Offset this to give the left a smaller target (going backwards is less
            // common)
