...
public class ClientSideEffects {
    ...
    public static void onEntityGeneratedMessage(UUID entityId,
            String uncleanEntityMessageResponse) {
        ServerPlayerEntity player = getChatData(entityId).getPlayer();
    ...
    public static void onCharacterSheetGenerated(UUID entityId, String characterSheet,
        ...
        if (characterName.equals("N/A")) {
            throw new RuntimeException(
                    "Generated \"\" or \"N/A\" as a character name! This should have been handled outside");
        }
    ...
    public static void onLLMGenerateError(UUID entityId, String errMsg) {
        LOGGER.error("Error processing LLM response, clearing msg. Error:", errMsg);
    ...
    public static void sendChatAsEntity(UUID entityId, String message) {
        ServerPackets.BroadcastEntityMessage(new EntityChatDataLight(entityId, message, 0, ChatStatus.DISPLAY, ChatSender.ASSISTANT, getChatData(entityId).characterSheet, getChatData(entityId).players));
    }
    ...
    public static void setStatusUsingParamsFromChatData(UUID entityId, ChatStatus status){
        ChatMessage topMessage = getChatData(entityId).previousMessages.get(getChatData(entityId).previousMessages.size()-1);
        ...
        // broadcast
        ServerPackets.BroadcastEntityMessage(new EntityChatDataLight(entityId, topMessage.message, getChatData(entityId).currentLineNumber, status, topMessage.sender, getChatData(entityId).characterSheet, getChatData(entityId).players));
    }
    ...
    public static void setLineNumberUsingParamsFromChatData(UUID entityId, int lineNumber){
        ...
        // broadcast
        ChatMessage topMessage = getChatData(entityId).previousMessages.get(getChatData(entityId).previousMessages.size()-1);
        ServerPackets.BroadcastEntityMessage(new EntityChatDataLight(entityId, topMessage.message, getChatData(entityId).currentLineNumber, getChatData(entityId).status, topMessage.sender, getChatData(entityId).characterSheet, getChatData(entityId).players));
    }
}
