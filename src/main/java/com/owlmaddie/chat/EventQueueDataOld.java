...
public class EventQueueDataOld {
    UUID entityId;
    Entity entity;
    ...
    private boolean needToGenCharacter() {
        LOGGER.info(String.format("NEEDTOGENCHAR characterName: %s  ", characterName ));
        ...
        LOGGER.info(String.format("NEEDTOGENCHAR propName: %s  ", name ));
        if (name == null) {
            return true;
        }
        characterName = name;
